	...
	Drupal.behaviors.facturacion_gt = {
		attach: function (context) {
			...
			const productosFields = context.querySelectorAll(
				"#productos_composite_table input.form-element--api-entity-autocomplete"
			); // Campos de "producto"
			const serviciosFields = context.querySelectorAll(
				"#servicios_composite_table input.form-element--api-entity-autocomplete"
			); // Campos de "servicio de alquiler"
			const cantidadProductosFields = context.querySelectorAll(
				'#productos_composite_table input[name*="[cantidad]"]'
			); // Campos de "cantidad"
			const diasServiciosFields = context.querySelectorAll(
				'input[name*="[horas]"]'
			); // Campos de "Dias"
						...
						if (uid) {
							fetch(`/facturacion-gt/user-data/${uid}`)
								...
								.then((data) => {
									...
									const nombreField = context.querySelector(
										"#edit-nombre-o-razon-social"
									);
									const identificacionField = context.querySelector(
										"#edit-identificacion"
									);
									const tipoPersonaField = context.querySelector(
										"#edit-tipo-de-persona"
									);
							...
							if (pid) {
								fetch(`/facturacion-gt/product-data/${pid}`)
									...
									.then((data) => {
										...
										const precioField = field
											.closest("tr")
											.querySelector('input[name*="[precio]"]');
										const cantidadField = field
											.closest("tr")
										...
										if (subtotalField) {
											subtotalField.value =
												precioField.value * cantidadField.value;
										}
				...
				serviciosFields.forEach((field) => {
					...
					if (!field.hasAttribute("data-autofill-initialized")) {
						...
						field.addEventListener("change", function (event) {
							...
							if (sid) {
								fetch(`/facturacion-gt/servicios-data/${sid}`)
									.then((response) => {
										if (!response.ok) {
											throw new Error(
									...
									.then((data) => {
										...
										if (subtotalField) {
											subtotalField.value = data.field_precio;
										}
									})
									.catch((error) => {
										console.error(
				...
				diasServiciosFields.forEach((field) => {
					...
					if (!field.hasAttribute("data-autofill-initialized")) {
						...
						field.addEventListener("change", function (event) {
							...
							if (precioField) {
								subtotalField.value = precioField.value * field.value;
							}
						});
					}
				});
